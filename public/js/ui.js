...
function showNextToast() {
  ...
  toast.className = "fixed top-4 left-1/2 -translate-x-1/2 px-6 py-3 rounded-lg shadow-lg transform transition-all duration-300 ".concat(bgColor, " text-white z-50");
...
function renderSearchHistory() {
  ...
  history.forEach((item) => {
    ...
    if (item.timestamp) {
      ...
      tag.title = "搜索于: ".concat(date.toLocaleString());
    }
...
function formatTimestamp(timestamp) {
  ...
  if (diff < 36e5) {
    ...
    return minutes <= 0 ? "刚刚" : "".concat(minutes, "分钟前");
  }
  if (diff < 864e5) {
...
function loadViewingHistory() {
  ...
  historyList.innerHTML = history.map((item) => {
    ...
    const episodeText = item.episodeIndex !== void 0 ? "第".concat(item.episodeIndex + 1, "集") : "";
    let progressHtml = "";
    if (item.playbackPosition && item.duration && item.playbackPosition > 10 && item.playbackPosition < item.duration * 0.95) {
      ...
      progressHtml = '\n                <div class="history-progress">\n                    <div class="progress-bar">\n                        <div class="progress-filled" style="width:'.concat(percent, '%"></div>\n                    </div>\n                    <div class="progress-text">').concat(formattedTime, " / ").concat(formattedDuration, "</div>\n                </div>\n            ");
    }
    const safeURL = encodeURIComponent(item.url);
    return '\n            <div class="history-item cursor-pointer relative group" onclick="playFromHistory(\''.concat(item.url, "', '").concat(safeTitle, "', ").concat(item.episodeIndex || 0, ", ").concat(item.playbackPosition || 0, ')">\n                <button onclick="event.stopPropagation(); deleteHistoryItem(\'').concat(safeURL, '\')" \n                        class="absolute right-2 top-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 text-gray-400 hover:text-red-400 p-1 rounded-full hover:bg-gray-800 z-10"\n                        title="删除记录">\n                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">\n                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>\n                    </svg>\n                </button>\n                <div class="history-info">\n                    <div class="history-title">').concat(safeTitle, '</div>\n                    <div class="history-meta">\n                        <span class="history-episode">').concat(episodeText, "</span>\n                        ").concat(episodeText ? '<span class="history-separator mx-1">·</span>' : "", '\n                        <span class="history-source">').concat(safeSource, "</span>\n                    </div>\n                    ").concat(progressHtml, '\n                    <div class="history-time">').concat(formatTimestamp(item.timestamp), "</div>\n                </div>\n            </div>\n        ");
  }).join("");
  if (history.length > 5) {
  ...
  try {
    ...
    if (historyRaw) {
      ...
      if (historyItem && historyItem.episodes && Array.isArray(historyItem.episodes)) {
        ...
        console.log("从历史记录找到视频 ".concat(title, " 的集数数据:"), episodesList.length);
      }
    }
    ...
    if (episodesList.length > 0) {
      ...
      console.log("已将剧集列表保存到localStorage，共 ".concat(episodesList.length, " 集"));
    }
    const positionParam = playbackPosition > 10 ? "&position=".concat(Math.floor(playbackPosition)) : "";
    if (url.includes("?")) {
      ...
      const playerUrl = "player.html?url=".concat(encodeURIComponent(url), "&title=").concat(encodeURIComponent(title), "&index=").concat(episodeIndex).concat(positionParam);
      showVideoPlayer(playerUrl);
    }
  } catch (e) {
    console.error("从历史记录播放失败:", e);
    const simpleUrl = "player.html?url=".concat(encodeURIComponent(url), "&title=").concat(encodeURIComponent(title), "&index=").concat(episodeIndex);
    ...
    if (existingIndex !== -1) {
      ...
      if (videoInfo.episodes && Array.isArray(videoInfo.episodes) && videoInfo.episodes.length > 0) {
        if (!existingItem.episodes || !Array.isArray(existingItem.episodes) || existingItem.episodes.length !== videoInfo.episodes.length) {
          console.log('更新 "'.concat(videoInfo.title, '" 的剧集数据: ').concat(videoInfo.episodes.length, "集"));
          existingItem.episodes = videoInfo.episodes.slice();
        }
      }
      history.splice(existingIndex, 1);
      history.unshift(existingItem);
    } else {
      const newItem = __spreadProps(__spreadValues({}, videoInfo), {
        timestamp: Date.now()
      });
      if (videoInfo.episodes && Array.isArray(videoInfo.episodes)) {
        newItem.episodes = videoInfo.episodes.slice();
        console.log('保存新视频 "'.concat(videoInfo.title, '" 的剧集数据: ').concat(videoInfo.episodes.length, "集"));
      } else {
        newItem.episodes = [];
      }
...
function navigateToTab(pageId) {
  ...
  const targetPage = document.getElementById("page-".concat(pageId));
  if (targetPage) {
