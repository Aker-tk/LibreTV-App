async function search() {
  ...
  try {
    ...
    const searchPromises = selectedAPIs.map(async (apiId) => {
      ...
      try {
        ...
        if (!data || !data.list || !Array.isArray(data.list)) {
          console.warn(`API ${apiId} (${apiUrl}) 返回的数据格式不符合预期 (缺少 list 数组):`, data);
          return [];
        }
            ...
            const pagePromise = (async () => {
              try {
                ...
                if (!pageData || !pageData.list || !Array.isArray(pageData.list)) {
                  console.warn(`API ${apiId} (${pageUrl}) 分页返回的数据格式不符合预期 (缺少 list 数组):`, pageData);
                  return [];
                }
    ...
    const safeResults = allResults.map((item) => {
      ...
      const sourceInfo = item.source_name ? `<span class="bg-[#222] text-xs px-1.5 py-0.5 rounded-full">${item.source_name.replace(/</g, "<").replace(/>/g, ">")}</span>` : "";
      const sourceCode = item.source_code || "";
      const apiUrlAttr = item.api_url ? `data-api-url="${item.api_url.replace(/"/g, '"')}"` : "";
      const hasCover = item.vod_pic && item.vod_pic.startsWith("http");
      return `
                <div class="card-hover bg-[#111] rounded-lg overflow-hidden cursor-pointer transition-all hover:scale-[1.02] h-full shadow-sm hover:shadow-md" 
                     onclick="if(typeof showDetails === 'function') showDetails('${safeId}','${safeName}','${sourceCode}'); else console.error('showDetails not defined')" ${apiUrlAttr}>
