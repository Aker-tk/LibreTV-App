    ...
    const searchPromises = selectedAPIs.map(async (apiId) => {
      ...
      try {
        ...
        if (!data || !data.list || !Array.isArray(data.list)) {
          console.warn("API ".concat(apiId, " (").concat(apiUrl, ") 返回的数据格式不符合预期 (缺少 list 数组，即使在备用后):"), data);
          return [];
        }
        const results = data.list.map((item) => {
          ...
          return __spreadProps(__spreadValues({}, item), {
            source_name: apiName,
            source_code: apiId,
            api_url: apiId.startsWith("custom_") ? (_a = getCustomApiInfo(apiId.replace("custom_", ""))) == null ? void 0 : _a.url : void 0
          });
        });
            ...
            const pagePromise = (async () => {
              ...
              try {
                ...
                if (!pageData || !pageData.list || !Array.isArray(pageData.list)) {
                  console.warn("API ".concat(apiId, " (").concat(pageUrl, ") 分页返回的数据格式不符合预期 (缺少 list 数组，即使在备用后):"), pageData);
                  return [];
                }
